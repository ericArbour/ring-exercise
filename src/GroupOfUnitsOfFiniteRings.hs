...
groupOfUnits =
  let ringElems = [minBound .. maxBound]
      ...
      inversesMap = Map.fromList $ inversePairs ++ map (\(a,b) -> (b,a)) inversePairs
  in GroupOfUnits { _elems = units, _inverses = inversesMap}
  where areInverses (a,b) = a Ring.* b == b Ring.* a && a Ring.* b == Ring.one
        getAllPairs [] = []
        getAllPairs [_] = []
...
instance ( Ring.C a, Ord a ) => Group (UnitGroupElem a) where
  invert (UnitGroupElem groups a) = UnitGroupElem groups inverse
    where inverse = case Map.lookup a $ _inverses groups of
